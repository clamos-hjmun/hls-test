...
router.get("/video-stream", async (_: Request, res: Response) => {
  try {
...
// 비디오 추출 요청 처리
router.post("/extract-video", async (req: Request, res: Response) => {
...
// 비디오 병합 API 엔드포인트
router.post("/merge-video", async (req: Request, res: Response) => {
  ...
  try {
    const updatedRanges = ranges.map(({ start, end }: { start: number; end: number }) => ({
      start,
...
// 비디오 병합 함수
const mergeVideo = (hlsUrl: string, ranges: { start: number; duration: number }[], outputPath: string) => {
...
const getVideoSegments = async (directory: string): Promise<string[]> => {
...
const sendFileIfExists = (filePath: string, res: Response, errorMessage: string) => {
...
const extractVideo = (hlsUrl: string, startTime: number, duration: number, outputPath: string) => {
  return new Promise<void>((resolve, reject) => {
    ffmpeg(hlsUrl)
